...
# Input/Output paths
INPUT_PATH = "cse476_final_project_submission/cse_476_final_project_test_data.json"
OUTPUT_PATH = "cse476_final_project_submission/cse_476_final_project_test_predictions.json"
...
def generate_predictions(limit=100):
    global results
    ...
    agent = Agent()
    print(f"Generating predictions for {len(remaining_data)} remaining items using Composite Strategy...")
    ...
    for i, item in enumerate(tqdm(remaining_data, initial=processed_count, total=len(data))):
        ...
        try:
            # Solve
            # The agent automatically picks the best strategy (Composite for math, Reflection for coding)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--limit", type=int, default=100, help="Limit number of items for testing")
    args = parser.parse_args()
    
    generate_predictions(limit=args.limit)
